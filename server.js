...
db.serialize(() => {
  ...
  // DEV_ROOT=true のとき root を admin で用意・補正
  if (DEV_ROOT) {
...
// ===== ユーティリティ =====
function signToken(payload){ return jwt.sign(payload, JWT_KEY, { expiresIn: '24h' }); }
function verifyToken(tok){ return jwt.verify(tok, JWT_KEY); }
  ...
  if (DEV_ROOT && username === 'root' && password === 'root') {
    db.get(`SELECT * FROM users WHERE username='root'`, (e, u) => {
      ...
      db.run(`INSERT INTO users (username,email,password,role) VALUES ('root', ?, 'root', 'admin')`,
        [DEV_ROOT_EMAIL], function(err){ if(err) return res.status(500).json({error:'Database error'}); finish(this.lastID,'admin'); });
    });
    return;
  }
...
app.put('/api/me', requireAuth, (req, res) => {
  ...
  // メールは users テーブルにも保存（ゆるい）
  if (p.email) {
    db.run(`UPDATE users SET email=? WHERE id=?`, [p.email, uid]);
  }
  // upsert 相当（存在しなければ insert、あれば update）
...
app.put('/api/me/password', requireAuth, (req, res) => {
  ...
  db.get(`SELECT password FROM users WHERE id=?`, [uid], (e, row) => {
    ...
    let ok = false;
    if (row.password && row.password.length > 20) { // ハッシュっぽい
